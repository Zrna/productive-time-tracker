  ...
  return (
    <div className="w-2/3 h-[85dvh] overflow-auto flex flex-col gap-4">
      {timeEntries?.data.map((entry) => {
        const serviceName = services?.data.find((service) => service.id === entry.relationships.service.data.id)
          ?.attributes.name;

        return (
          <div className="relative" key={entry.id}>
            {timeEntryDeletingId === entry.id && <p className="absolute left-2/4 top-2/4">Deleting...</p>}
            <div
            ...
            >
              ...
              <p className="cursor-pointer text-red-600 w-fit" onClick={() => onDelete(entry.id)}>
                Delete
              </p>
            </div>
          </div>
